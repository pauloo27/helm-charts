...
- **Port Configuration** - Single `httpPort` parameter controls:
  - Container port
  - Service port
  - Health probe port
...
|-----------|---------|-------------|
| `httpPort` | `8080` | HTTP port for the application (used across container, service, and probes) |
...
- Triggered automatically when version tags (e.g., `v1.0.0`) are pushed to this repo
...
- Chart reference: `oci://ghcr.io/pauloo27/helm-charts/sample-java-api:v1.0.0`
...
- **Single Port**: Application only exposes one HTTP port, configured via
  `httpPort` parameter that's reused across all relevant resources
